...
class MigrationTest(object):
            ...
            if vm_state == "nonexist":
                if virsh.domain_exists(vm.name, uri=uri, debug=True):
                    raise exceptions.TestFail(
                        f"Expected the VM not to exist, bug it exists."
                    )
            else:
                if not libvirt.check_vm_state(
                    vm.name, vm_state, uri=dest_uri, debug=True
                ):
    ...
    def ping_vm(self, vm, params, uri=None, ping_count=10, ping_timeout=60):
        ...
        if uri and uri != "qemu:///system":
            ...
            vm_ip[vm.name] = vm.get_address(session=server_session, timeout=240)
    ...
    def thread_func_migration(
        ...
        try:
            ...
            if self.ret.exit_status != 0:
                LOG.debug(
                    "Migration to %s returns failed exit status %d",
                    desturi,
                    self.ret.exit_status,
                )
                is_error = True
    ...
    def migrate_pre_setup(self, desturi, params, cleanup=False, ports="49152:49216"):
        """
        ...
        try:
            dest_ip = re.search(r"//.*/", desturi, re.I).group(0).strip("/").strip()
            ...
            if (
                ...
                if cmd_output[0] == 0:
                    ...
                    raise exceptions.TestError(
                        "Failed to get cpuinfo of remote " "server", cmd_output[1]
                    )
    ...
    def do_migration(
        ...
        **args,
            ...
            if isinstance(funcs_to_run, list):
                for one_func in funcs_to_run:
                    if isinstance(one_func, dict):
                        ...
                        raise exceptions.TestError(
                            "Only a dict element is " "supported in funcs_to_run"
                        )
        ...
        def _run_complex_func(vm, one_func, virsh_event_session=None):
            ...
            wait_for_after_event_timeout = one_func.get(
                "wait_for_after_event_timeout", "30"
            )
            if after_event and not virsh_event_session:
                raise exceptions.TestError(
                    "virsh session for collecting domain " "events is not provided"
                )

            if after_event:
                LOG.debug(
                    "Below events are received:" "%s",
                    virsh_event_session.get_stripped_output(),
                )
                if not utils_misc.wait_for(
                    lambda: re.findall(
                        after_event, virsh_event_session.get_stripped_output()
                    ),
                    wait_for_after_event_timeout,
                ):
                    raise exceptions.TestError(
                        "Unable to find " "event {}".format(after_event)
                    )
            ...
            if before_event and re.findall(
                ...
                raise exceptions.TestError(
                    ...
                    "'{}', but the event has "
                    "been received".format(func, before_event)
                )
            ...
            for one_func in funcs_to_run:
                if isinstance(one_func, (types.FunctionType, types.MethodType)):
                    if not before_pause:
                        ...
                        LOG.error(
                            "Only support to run the function " "after guest is paused"
                        )
        ...
        def _do_orderly_migration(
            ...
            **args,
            ...
            migration_thread = threading.Thread(
                ...
                args=(vm, desturi, options, ignore_status, virsh_opt, extra_opts),
            )
        ...
        if migration_type == "orderly":
            ...
            for vm in vms:
                thread1 = threading.Thread(
                    target=self.thread_func_migration, args=(vm_remote, srcuri, options)
                )
                thread2 = threading.Thread(
                    target=self.thread_func_migration, args=(vm, desturi, options)
                )
                thread1.start()
                thread2.start()
    ...
    def wait_for_migration_start(
        self, vm, state="paused", uri=None, migrate_options="", timeout=60
    ):
        """
        ...
        if migrate_options.count("--dname"):
            ...
            dest_vm_name = migrate_options_list[
                migrate_options_list.index("--dname") + 1
            ]
        else:
    ...
    def update_virsh_migrate_extra_args(self, params):
        ...
        if func_params_exists:
            if params.get("action_during_mig_params"):
                extra_args.update(
                    {"func_params": eval(params.get("action_during_mig_params"))}
                )
            else:
    ...
    def do_cancel(self, sig=signal.SIGKILL):
        ...
        if utils_misc.safe_kill(pid, sig):
            ...
            raise exceptions.TestError("Fail to cancel migration: [%s]" % pid.strip())
    ...
    def set_migratepostcopy(self, vm_name, uri=None):
        ...
        cmd = "event --loop --all"
        virsh_event_session = virsh.VirshSession(
            virsh_exec=virsh.VIRSH_EXEC, auto_close=True, uri=uri
        )
        virsh_event_session.sendline(cmd)

        if not utils_misc.wait_for(
            lambda: not virsh.migrate_postcopy(
                vm_name, uri=uri, debug=True
            ).exit_status,
            10,
        ):
        ...
        if not utils_misc.wait_for(
            lambda: re.findall(exp_str, virsh_event_session.get_stripped_output()), 30
        ):
            raise exceptions.TestError("Unalbe to find event {}".format(exp_str))
    ...
    def control_migrate_speed(self, vm_name, to_speed=1, mode="precopy"):
        ...
        def _set_speed(extra_option=""):
            ...
            old_speed = virsh.migrate_getspeed(
                vm_name, extra=extra_option, **virsh_args
            )
            LOG.debug(
                "Current %s migration speed is %s " "MiB/s\n",
                extra_option,
                old_speed.stdout_text.strip(),
            )
            LOG.debug("Set %s migration speed to %d " "MiB/s\n", extra_option, to_speed)
            virsh.migrate_setspeed(vm_name, to_speed, extra=extra_option, **virsh_args)

        if mode not in ["postcopy", "precopy", "both"]:
            raise exceptions.TestError(
                "'mode' only supports " "'postcopy', 'precopy', 'both'"
            )
        warning_msg = (
            "libvirt version should be larger than or equal to "
